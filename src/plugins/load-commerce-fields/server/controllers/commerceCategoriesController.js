...
module.exports = ({ strapi }) => ({
  async getCategories(ctx) {
    try {
      ...
      const categories =
        await commerceCategoriesService.getCommerceCategories();
      return categories;
    } catch (error) {
  ...
  async searchCategories(ctx) {
    try {
      ...
      const categories =
        await commerceCategoriesService.searchCategories(query);
      return categories;
    } catch (error) {
  ...
  async getProducts(ctx) {
    try {
      ...
      const commerceCategoriesService = strapi
        ...
        .service("commerceCategoriesService");
      const products =
        await commerceCategoriesService.getProductsByCategory(categoryUid);
      return products;
    } catch (error) {
